...
type Mutation {
    ...
    previewCampaignPlan(specification: CampaignPlanSpecification!): CampaignPlan!
...
input CreateCampaignInput {
    ...
    # An optional reference to a campaign plan that was previewed before this mutation.
    # If set and the plan is still cached, it will be used.
    # The CampaignPlan associated with the campaign when it was created.
    # Otherwise the campaign plan will be recomputed from the specification.
    plan: ID
}
...
input CampaignPlanSpecification {
    ...
    #
    # Schema for comby:
    # { matchTemplate: string, rewriteTemplate: string }
...
type CampaignPlan implements Node {
    ...
    # Whether this campaign plan has finished planning all changesets and their diffs.
    # If false, changesets and repositoryDiffs are not complete yet.
    # Query the campaign plan by its ID again to see its latest state.
    isComplete: Boolean!
    ...
    repositoryDiff(first: Int): PreviewRepositoryDiffConnection!
}
...
type Changeset implements Node {
    ...
    # if already opened on the code host.
    externalURL: ExternalLink

    # The review state of this changeset.
    # PENDING for planned changesets.
